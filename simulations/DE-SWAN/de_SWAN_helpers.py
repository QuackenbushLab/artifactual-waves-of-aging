            ...
            try:
                if wilcoxon:
                    ...
                    if has_covariates:
                        ...
                        p = anova_table.loc["age_group", "PR(>F)"] 
                    else:
                        rhs_terms = ["age_group"]
                        formula = "expression ~ " + " + ".join(rhs_terms)
...
def run_all_de_swan(mols_num, exp_df, 
                    window=20, 
                    age_col="age", 
                    min_age=25, 
                    max_age=75, 
                    wilcoxon_indicator=False, 
                    covariates=None):
    if covariates is None:
        covariates = []

    exp_df_reshaped = []
    base_cols = [age_col] + covariates

    for mol_idx in range(mols_num):
        mol_df = exp_df[base_cols + [f"mol_{mol_idx}"]].copy()
        mol_df["molecule"] = mol_idx
        mol_df.rename(
            columns={
                f"mol_{mol_idx}": "expression",
                age_col: "age"
            },
            inplace=True
        )
